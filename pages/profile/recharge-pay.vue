<template>
  <view class="container">
    <view class="pay-card">
      ...
      <text class="pay-title">{{ channelName }}</text>
      <text class="pay-subtitle">{{ label('mockCheckout') }}</text>
      ...
      <view class="info-row">
        <text class="info-label">{{ label('method') }}</text>
        <text class="info-value">{{ channelName }}</text>
      </view>
...
export default {
  ...
  methods: {
    ...
    pay() {
      ...
      request({
        ...
        success: (res) => {
          ...
          this.paid = true
          uni.showToast({ title: this.label('rechargeSuccess'), icon: 'success' })
          setTimeout(() => {
    ...
    goBack() {
      goBackOrFallback('/pages/profile/recharge')
    }
  }
}
</script>
