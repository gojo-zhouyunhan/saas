    ...
    <view class="sell-form">
      <view class="section">
        ...
        <view class="image-uploader">
          ...
          <view class="upload-item" v-for="(image, index) in images" :key="index">
            ...
            <text class="delete-icon" @click="deleteImage(index)">x</text>
          </view>
        </view>
      </view>
      ...
      <view class="section">
        ...
        <textarea
          ...
          placeholder-style="color: #94A3B8;"
        />
        <text class="helper-text">AI 会把描述整理成一组车辆报告卡片，方便卖家快速检查亮点和买家顾虑。</text>
      </view>
      ...
      <view v-if="aiPreview" class="section ai-preview">
        <view class="preview-header">
          ...
          <text class="preview-badge">REPORT</text>
        </view>
...
export default {
  ...
  methods: {
    ...
    generateAiPreview() {
      ...
      uni.request({
        url: `${API_BASE_URL}/api/ai/report/generate-sell-visualization`,
        method: 'POST',
        data: {
        ...
        success: (res) => {
          ...
          this.aiPreview = null
          this.aiError = payload.message || 'AI 解析失败，请稍后重试'
        },
        fail: (err) => {
          this.aiLoading = false
          this.aiPreview = null
