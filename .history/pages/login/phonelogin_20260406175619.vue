...
export default {
  ...
  methods: {
    ...
    getCode() {
      ...
      // 发送验证码请求
      uni.request({
        url: `http://192.168.1.133:8081/api/user/sendcode?phone=${this.form.phone}`,
        method: 'GET',
        success: (res) => {
    ...
    login() {
      ...
      // 发送验证码验证请求
      uni.request({
        url: `http://192.168.1.133:8081/api/user/checkcode?phone=${this.form.phone}&code=${this.form.code}`,
        method: 'GET',
        success: (res) => {
          if (res.statusCode === 200 && res.data.code === 200) {
            uni.showToast({
